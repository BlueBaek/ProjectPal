...
#include "PalSkill_GrassTornados.generated.h"

class APJ_GrassTornado;

UCLASS()
class PROJECTPAL_API UPalSkill_GrassTornados : public UPalSkillExecution
{
	GENERATED_BODY()
...
protected:
	...
	// ✅ 좌/우 오프셋 (RightVector 기준)
	UPROPERTY(EditDefaultsOnly, Category="GrassTornados")
	float SpawnSideOffset = 100.f;
	...
	UPROPERTY()
	TObjectPtr<APJ_GrassTornado> SpawnedLeftTornado = nullptr;

	UPROPERTY()
	TObjectPtr<APJ_GrassTornado> SpawnedRightTornado = nullptr;

private:
	// SideSign: -1 = Left, +1 = Right
	APJ_GrassTornado* SpawnTornado(float SideSign);
};
